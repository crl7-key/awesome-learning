...
// 解析最外层的error_log指令,设置新的log对象
static char *ngx_error_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

// 设置日志的级别
static char *ngx_log_set_levels(ngx_conf_t *cf, ngx_log_t *log);

// 日志对象按级别低到高,即NGX_LOG_DEBUG=>NGX_LOG_STDERR,按level降序
...
// error_log指令只能出现在最外部,不能在event{}http{}里,http/stream等定义了自己的error_log指令
...
// 默认不使用标准流记录日志,在nginx.c里置0
ngx_uint_t              ngx_use_stderr = 1;


// 错误级别与字符串的对应数组, ngx_log_set_levels
...
// 调试用的级别,只打印某些特殊子系统的日志,在nginx网站没有很好地文档化,ngx_log_set_levels
...
#endif
{
    ...
    if (err) {
     ...
     // 记录错误日志时可以执行的回调函数,参数是消息缓冲区里剩余的空间,只有高于debug才会执行
    ...
    // 是否要针对某些连接打印调试日志,指令debug_connection
    debug_connection = (log->log_level & NGX_LOG_DEBUG_CONNECTION) != 0;

    // 对整个日志链表执行写入操作
    while (log) {

        // log消息级别低,不需要记录日志,直接退出循环,因为链表是level降序的,避免了遍历整个链表,提高效率, 如果是针对某些连接打印调试日志那么无视日志级别
        if (log->log_level < level && !debug_connection) {
            break;
        }

        // log对象有专用的写函数指针,例如syslog, 那么就不写文件,调用函数写日志,可以定制writer函数,实现特殊的写日志行为,如rotate
        if (log->writer) {
        ...
        // 写错误日志消息到关联的文件,实际上就是系统调用write,见ngx_files.h
        n = ngx_write_fd(log->file->fd, errstr, p - errstr);

        // 写入失败,且错误是磁盘满,记录时间,避免磁盘满时反复写入
    ...
    // 前面输出的是基本的信息：当前时间+[错误级别]+pid#tid: 调整字符串指针
...
ngx_log_init(u_char *prefix)
{
    ...
    if (name[0] != '/') {
        ...
        // 取路径前缀的长度,默认是NGX_CONF_PREFIX,即/usr/local/nginx
        if (prefix) {
    ...
    // 打开有前缀的日志文件,因为使用了APPEND,所以多进程写文件是安全的
...
ngx_log_open_default(ngx_cycle_t *cycle)
{
    ...
    // 在日志链表里找到一个使用文件的日志对象,有就说明已经用指令设置了日志
    ...
    // 打开文件,注意没有前缀,默认使用conf_prefix/prefix
...
ngx_log_set_levels(ngx_conf_t *cf, ngx_log_t *log)
{
    ...
    // 逐个检查file之后的level参数,允许有多个debug level级别
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        // 检查是否是debug level/debug_core/debug_alloc等

...
// 从头开始查找所有模块,最后找到了一个未使用的序号,通常是数组里的最后一个位置
static ngx_uint_t ngx_module_index(ngx_cycle_t *cycle);
// 从头开始查找特定类型的模块,如果这个序号被已有的模块使用,那么序号加1,再重新查找,例如，一开始所有模块的序号都是-1，那么返回0，之后就是1、2、3,对于静态模块很简单，返回值就是index
static ngx_uint_t ngx_module_ctx_index(ngx_cycle_t *cycle, ngx_uint_t type,
    ngx_uint_t index);

// 在ngx_preinit_modules里统计得到静态模块的总数,之后加上128,就是模块数量的上限
ngx_uint_t         ngx_max_module;
// 模块计数器，模块数组的最后一个可用序号
static ngx_uint_t  ngx_modules_n;

// 在main()里调用,计算所有的静态模块数量
ngx_int_t
...
// 在main() -> ngx_init_cycle里调用,内存池创建一个数组,可以容纳所有的模块,大小是ngx_max_module + 1
// 拷贝脚本生成的静态模块数组到本cycle, 拷贝模块序号计数器到本cycle,完成cycle的模块初始化
ngx_int_t
ngx_cycle_modules(ngx_cycle_t *cycle)
{
     ...
     */
    // 内存池创建一个数组,可以容纳所有的模块，大小是ngx_max_module + 1,使用的是ngx_pcalloc,内容全是0,即以null表示数组结束
...
// 在main() -> ngx_init_cycle里调用,仅调用一次,调用所有模块的init_module函数指针,初始化模块,不使用全局的ngx_modules数组,而是使用cycle里的,这时可能已经加载了一些动态模块
ngx_int_t
...
// 在ngx_event.c等调用,在解析配置块时,这时应该已经加载了动态模块,在模块数组里与静态模块无区别,得到cycle里所有的事件/http/stream模块数量,设置某类型模块的ctx_index
// type是模块的类型，例如NGX_EVENT_MODULE,返回此类型模块的数量
ngx_int_t
ngx_count_modules(ngx_cycle_t *cycle, ngx_uint_t type)
{
    ...
    /* count appropriate modules, set up their indices */
     // 遍历cycle里的模块数组，只关注特定类型的模块,设置其索引
    for (i = 0; cycle->modules[i]; i++) {
        ...
        if (module->ctx_index != NGX_MODULE_UNSET_INDEX) {
         ...
         // 从头开始查找特定类型的模块,如果这个序号被已有的模块使用,那么序号加1，再重新查找,例如，一开始所有模块的序号都是-1,那么返回0,之后就是1、2、3,对于静态模块很简单,返回值就是next
        /* search for some free index */
        // 通常情况下模块都是没有序号的(即-1),调用ngx_module_ctx_index获取一个序号
        module->ctx_index = ngx_module_ctx_index(cycle, type, next);
        // 更新max
        if (module->ctx_index > max) {
            max = module->ctx_index;
        }
        // 更新next,序号加1,准备给下一个模块
     ...
     */
    // 确保返回的数字足够用于先前的,周期也一样,否则数字会有问题,将存储在全局变量中(以前是)
    ...
    // 标志位,cycle已经完成模块的初始化,不能再添加模块,防止加载其他模块
    cycle->modules_used = 1;
    // +1是为了最后一个元素放null表示数组结束
    return max + 1;
}

// 使用模块里的各种信息进行检查,只有正确的才能加载,否则报错
// file参数仅打错误日志使用
ngx_int_t
    ...
    char **order)
{
    ...
    if (ngx_strcmp(module->signature, NGX_MODULE_SIGNATURE) != 0) {
     ...
     // 遍历cycle里的模块数组里，检查是否有重名的,  每个模块的名字都不能相同 
     ...
     */
    // 如果模块先前未加载,就分配一个全局索引,不是ctx_index
     ...
     */
    // 将模块放入cycle-> modules数组, cycle->modules_n是模块计数器,是数组里的第一个空位置
    before = cf->cycle->modules_n;
    // 模块顺序只对http filter模块有意义,其他模块不会走这里
    if (order) {
        for (i = 0; order[i]; i++) {

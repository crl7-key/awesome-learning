...
**/
// 操作系统提供的底层数据收发接口,ngx_posix_init.c里初始化为linux的底层接口,在epoll模块的ngx_epoll_init里设置
ngx_os_io_t  ngx_io;

// 检查最多32个在可复用连接队列里的元素,设置为连接关闭c->close = 1;
// 调用事件的处理函数,里面会检查c->close,这样就会调用ngx_http_close_connection,释放连接,加入空闲链表,可以再次使用
    ...
    socklen_t socklen)
{
    ...
    // 管理本端口的udp客户端连接,保持udp连接,支持客户端发多包,但对于tcp连接来说浪费了点空间
#if !(NGX_WIN32)
...
// 为reuseport使用,拷贝了worker数量个的监听结构体,在ngx_stream_optimize_servers等函数创建端口时调用
ngx_int_t
ngx_clone_listening(ngx_cycle_t *cycle, ngx_listening_t *ls)
{
// configure脚本可以检测系统是否支持reuseport,使用宏来控制条件编译
    ...
    // ls在之前已经正确设置了若干参数,例如type/handler/backlog等等
    ols = *ls;

    ccf = (ngx_core_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_core_module);
    
    // ccf->worker_processes是nginx的worker进程数,拷贝了worker数量个的监听结构体
    // 注意从1开始,这样拷贝后总数就是worker数量个,最开始的一个就是被克隆的ls监听结构体
    for (n = 1; n < ccf->worker_processes; n++) {
        ...
        *ls = ols;
        ...
        // 设置worker的序号,被克隆的对象的worker是0
        // worker的使用是在ngx_event.c:ngx_event_process_init,只有worker id是本worker的listen才会enable
        ls->worker = n;
    }
...
// 创建socket, bind/listen ,在 ngx_cycle.c : init_cycle()里被调用,之后会调用ngx_configure_listening_sockets(),配置监听端口的rcvbuf/sndbuf等参数,调用setsockopt()
ngx_int_t
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (tries = 5; tries; tries--) {
        ...
        /* for each listening socket */
         // 遍历监听端口链表,如果设置了reuseport,那么一个端口会有一个worker克隆的结构体
        ls = cycle->listening.elts;
        for (i = 0; i < cycle->listening.nelts; i++) {
...
#if (NGX_HAVE_REUSEPORT)
            // 检查是否已经设置的标志位,只是用在inherited的时候,见注释,在init_cycle里设置
            if (ls[i].add_reuseport) {
            
                // 对于具有SO_REUSEPORT的多个套接字.必须设置
                // 在打开新套接字之前,在旧套接字上使用SO_REUSEPORT
                /*
...
// SO_REUSEPORT_LB目前仅在FreeBSD里有效
            ...
            // 从前一个nginx进程继承过来的,已经打开,所以也不需要再处理,在ngx_set_inherited_sockets里操作
            if (ls[i].inherited) {
        ...
        // 设置SO_REUSEPORT选项,这样多个进程可以打开相同的端口,由内核负责负载均衡
#if (NGX_HAVE_REUSEPORT)
...
// 配置监听端口的rcvbuf/sndbuf等参数,调用setsockopt(),在 ngx_init_cycle()里调用,在ngx_open_listening_sockets()之后
void
...
// linux的设置
        ...
        // deferred,只有socket上有数据可读才接受连接,由内核检查客户端连接的数据发送情况,减少了epoll的调用次数,可以提高性能
...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    // 从全局变量ngx_cycle里获取空闲链接,即free_connections链表,free_connections是空闲链表头指针
    // 检查最多32个在可复用连接队列里的元素,设置为连接关闭c->close = 1;调用事件的处理函数,里面会检查c->close,这样就会调用ngx_http_close_connection,释放连接,加入空闲链表,可以再次使用
    ...
    // 此时已经拿到了空闲连接,调整空闲链表的指针,复用data成员,空闲链表头指针指向链表里的下一个节点
    ...
    wev = c->write;
     ...
     // 清空连接对象,注意这时fd、sent、type等字段、计数器、标志都变成了0
...
{
    // 调整空闲链表的指针,复用data成员,free_connections是空闲链表头指针
...
ngx_close_connection(ngx_connection_t *c)
{
    ...
    if (!c->shared) {
        // 宏,实际上是ngx_event_actions.del_conn,在 event/ngx_event.h中定义
        if (ngx_del_conn) {
...
ngx_drain_connections(ngx_cycle_t *cycle)
{
    ...
    // 从全局变量ngx_cycle里获取空闲链接,即free_connections链表,free_connections是空闲链表头指针
    // 早期的nginx只检查32次,避免过多消耗时间,1.12版本改变了这个固定值
    ...
    for (i = 0; i < n; i++) {
        ...
        // 调用事件的处理函数,里面会检查c->close,这样就会调用ngx_http_close_connection,释放连接,加入空闲链表,可以再次使用,例如ngx_http_wait_request_handler
        c->read->handler(c->read);
    }
}

// 检查cycle里的连接数组,如果连接空闲则设置close标志位,关闭连接
void
    ...
    ngx_uint_t port)
{
    ...
    // 先检查c->local_socklen,即ls->sock_addr,监听端口的地址
    if (c->local_socklen) {

...
// 线程池模块的配置,里面是个数组,元素为ngx_thread_pool_t,由ngx_thread_pool_add在解析指令的ngx_thread_pool里添加
typedef struct {
    ngx_array_t               pools;
} ngx_thread_pool_conf_t;


// 线程池使用的任务队列,与ngx_queue不同,不是侵入式节点,而是首尾指针
typedef struct {
...
// 初始化线程池任务队列,first/last都是空,与ngx_queue_init不同
#define ngx_thread_pool_queue_init(q)                                         \
    (q)->first = NULL;                                                        \
    (q)->last = &(q)->first


// 描述一个线程池,与thread_pool指令对应,此结构体的实际定义在c文件里,外部不可见,深度定制则不方便.存储在ngx_thread_pool_conf_t里的数组里,核心成员是queue,存储待处理任务
struct ngx_thread_pool_s {
    
    // 互斥量,锁定互斥量,防止多线程操作的竞态,锁定操作waiting/queue/ngx_thread_pool_task_id
    ngx_thread_mutex_t        mtx;
    
    // 线程池里的待处理任务队列 ngx_thread_task_post把任务放入线程池,ngx_thread_pool_cycle消费任务
...
// 使用ngx_thread_pool_t结构体初始化线程池,在init_worker时被调用,创建互斥量、条件变量,根据配置的线程数量,创建线程,线程的执行函数是ngx_thread_pool_cycle,参数是线程池结构体
static ngx_int_t ngx_thread_pool_init(ngx_thread_pool_t *tp, ngx_log_t *log,
    ngx_pool_t *pool);

// 销毁线程池,使用一个要求线程结束的task,发给池里所有的线程,最后销毁条件变量和互斥量
...
// 线程池里每个线程执行的函数,无限循环,参数是线程池结构体,从待处理任务队列里获取任务,然后执行task->handler(task->ctx),处理完的任务加入完成队列
static void *ngx_thread_pool_cycle(void *data);

// 分发处理线程完成的任务,在主线程里执行,调用event->handler,即异步事件完成后的回调函数
...
// 进程开始时初始化,创建线程池,在ngx_single_process_cycle/ngx_worker_process_cycle里调用
static ngx_int_t ngx_thread_pool_init_worker(ngx_cycle_t *cycle);

// 调用ngx_thread_pool_destroy逐个销毁线程池,进程结束时被调用,清理线程池
static void ngx_thread_pool_exit_worker(ngx_cycle_t *cycle);

// 解析thread_pool指令,设置线程数和队列数（默认65535） 线程池模块属于core模块,只有一个指令,配置有名的线程池
...
ngx_module_t  ngx_thread_pool_module = {
    ...
    // 进程结束时被调用,清理线程池,调用ngx_thread_pool_destroy逐个销毁线程池    
...
// 处理完毕(handler(ctx))的任务都放到这里,使用ngx_thread_pool_done_lock保护
...
ngx_thread_pool_init(ngx_thread_pool_t *tp, ngx_log_t *log, ngx_pool_t *pool)
{
    ...
    // 要求必须有事件通知函数ngx_notify,否则多线程无法工作,调用系统函数eventfd,创建一个可以用于通知的描述符
    if (ngx_notify == NULL) {
    ...
    for (n = 0; n < tp->threads; n++) {
        ...
        // 线程的执行函数是ngx_thread_pool_cycle,参数是线程池结构体,线程创建后立即detach
...
// 要求线程结束的任务,调用pthread_exit,把任务需要处理的data置为0,表示线程结束
static void
...
// 创建一个线程任务结构体,参数size是用户数据ctx的大小,位于task之后,因为C的内存布局是平坦的,所以使用这种hack的方法来扩展task结构体
ngx_thread_task_t *
...
// 把任务放入线程池,由线程执行,锁定互斥量,防止多线程操作的竞态
// 如果等待处理的任务数大于设置的最大队列数,那么添加任务失败,操作完waiting、queue、ngx_thread_pool_task_id后解锁
ngx_int_t
ngx_thread_task_post(ngx_thread_pool_t *tp, ngx_thread_task_t *task)
{
    // event.active表示任务是否已经放入任务队列,如果event.active==1则ngx_thread_task_post失败
...
ngx_thread_pool_cycle(void *data)
{
    ...
    if (err) {
     ...
     // 无限循环,从待处理任务队列里获取任务,然后执行task->handler(task->ctx)
    for ( ;; ) {
        // 锁定互斥量,防止多线程操作的竞态
        if (ngx_thread_mutex_lock(&tp->mtx, tp->log) != NGX_OK) {
            return NULL;
        ...
        *ngx_thread_pool_done.last = task;
        ...
        // 确保内存操作按照正确的顺序工作的非阻塞的同步,迫使处理器来完成位于barrier前面的任何加载和存储操作,才允许它执行位于barrier之后的加载和存储操作
        ngx_memory_barrier();

        // 自旋锁解锁
        ngx_unlock(&ngx_thread_pool_done_lock);
        
        // 重要,使用event模块的通知函数,让主线程(nginx)的epoll触发事件,调用ngx_thread_pool_handler,分发处理线程完成的任务
...
ngx_thread_pool_handler(ngx_event_t *ev)
{
    ...
    // 把队列直接置空,即ngx_thread_pool_queue_init
...
ngx_thread_pool_exit_worker(ngx_cycle_t *cycle)
{
    ...
    // 只有worker和single进程才会有线程池,master/cache不处理请求,不使用线程池
    if (ngx_process != NGX_PROCESS_WORKER
        && ngx_process != NGX_PROCESS_SINGLE)
    {
        return;
    ...
    for (i = 0; i < tcf->pools.nelts; i++) {
         // 使用一个要求线程结束的task,发给池里所有的线程,最后销毁条件变量和互斥量

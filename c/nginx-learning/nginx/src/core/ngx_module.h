...
// 定义动态模块的签名，保证可以正确加载,使用了指针大小等宏，保证二进制一致性
#define NGX_MODULE_SIGNATURE_0                                                \
    ngx_value(NGX_PTR_SIZE) ","                                               \
    ngx_value(NGX_SIG_ATOMIC_T_SIZE) ","                                      \
    ngx_value(NGX_TIME_T_SIZE) ","

// 定义各种系统调用的支持程度,只有完全匹配即系统符合要求才能加载
#if (NGX_HAVE_KQUEUE)
...
// 重要的数据结构，定义nginx模块 重要的模块ngx_core_module/ngx_event_module/ngx_http_module, 在1.9.11之后有变化，改到了ngx_module.h，可以定义动态模块，使用了spare0等字段
struct ngx_module_s {
    ...
    char                 *name;      // 模块的名字，标识字符串，默认是空指针,由脚本生成ngx_module_names数组,然后在ngx_preinit_modules里填充,动态模块在ngx_load_module里设置名字
    ...
    void                 *ctx;       // 模块不同含义不同,通常是函数指针表，是在配置解析的某个阶段调用的函数
    ...
    ngx_int_t           (*init_module)(ngx_cycle_t *cycle); // 在ngx_init_cycle里被调用,在master进程里，fork出worker子进程之前,做一些基本的初始化工作，数据会被子进程复制

    ngx_int_t           (*init_process)(ngx_cycle_t *cycle);// 在ngx_single_process_cycle/ngx_worker_process_init里调用,在worker进程进入工作循环之前被调用,初始化每个子进程自己专用的数据
    ...
    // 下面8个成员通常用用NGX_MODULE_V1_PADDING填充,暂时无任何用处
...
typedef struct {
    ngx_str_t             name;
    void               *(*create_conf)(ngx_cycle_t *cycle);// create_conf函数返回的是void*指针
    char               *(*init_conf)(ngx_cycle_t *cycle, void *conf);
} ngx_core_module_t;

// 在main()中调用,计算所有的静态模块数量,ngx_modules是nginx模块数组，存储所有的模块指针，由make生成在objs/ngx_modules.c,这里赋值每个模块的index成员,ngx_modules_n保存了最后一个可用的序号,ngx_max_module是模块数量的上限
ngx_int_t ngx_preinit_modules(void);
// 在main() - > ngx_init_cycle里调用,内存池创建一个数组，可以容纳所有的模块，大小是ngx_max_module + 1,拷贝脚本生成的静态模块数组到本cycle,拷贝模块序号计数器到本cycle,完成cycle的模块初始化
ngx_int_t ngx_cycle_modules(ngx_cycle_t *cycle);
// 在main() - > ngx_init_cycle里调用,调用所有模块的init_module函数指针，初始化模块,不使用全局的ngx_modules数组，而是使用cycle里的
ngx_int_t ngx_init_modules(ngx_cycle_t *cycle);
// 返回类型模块的数量,在ngx_event.c等调用，在解析配置块时,得到cycle里所有的事件/http/stream模块数量,设置某类型模块的ctx_index，type是模块的类型，例如NGX_EVENT_MODULE
ngx_int_t ngx_count_modules(ngx_cycle_t *cycle, ngx_uint_t type);

// cycle->modules_n是模块计数器 如果超过最大数量则报错,使用模块里的各种信息进行检查，只有正确的才能加载,首先是版本号,必须一致,例如1.10的不能给1.9使用,比较签名字符串，里面是二进制兼容信息
// 查看cycle里的模块数组里是否有重名的 也就是说每个模块的名字都不能相同,模块还没有加载,那么就会给一个全局序号,不是ctx_index,把动态模块的指针加入cycle的模块数组,最后完成了一个动态模块的加载，放到了cycle模块数组里的合适位置
ngx_int_t ngx_add_module(ngx_conf_t *cf, ngx_str_t *file,
    ngx_module_t *module, char **order);

// nginx模块数组，存储所有的模块指针，由make生成在objs/ngx_modules.c,ngx_cycle_modules拷贝后就不再使用
extern ngx_module_t  *ngx_modules[];
// 模块数量的上限, 所有模块不能超过这个数量，ngx_module.c
extern ngx_uint_t     ngx_max_module;
// 模块的名字数组,由make生成在objs/ngx_modules.c,仅在ngx_preinit_modules里使用

...
static char *ngx_set_worker_processes(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
// 加载动态模块的指令,调用dlopen,不能在http/event中使用,需要在http/event之前，否则会因为module_used不允许加载动态模块
//      1. 打开动态库文件
//      2. 设置内存池销毁时的清理动作,关闭动态库
//      3. 使用"ngx_modules"获取动态库里的模块数组
//      4. 使用"ngx_module_names"获取取动态库里的模块名字数组 
//      5. 使用"ngx_module_order"取动态库里的模块顺序数组
//      6. 模块顺序只对http filter模块有意义，所以可以没有，不需要检查
//      7. 遍历动态里的模块模块数组，流程类似ngx_preinit_modules调用ngx_add_module添加模块
...
//  ngx_core_module定义的核心指令，都在main域配置，配置结构体是ngx_core_conf_t，定义在ngx_cycle.h
static ngx_command_t  ngx_core_commands[] = {
    // 守护进程daemon on|off 是否已守护进程方式运行，守护进程是脱离终端在后台运行的进程，脱离终端是避免进程执行过程中的打印在任何终端上面显示
    { ngx_string("daemon"),
    ...
    { ngx_string("master_process"),
      ...
      NULL },
    // nginx更新缓存时间的精度，如果设置了会定时发送sigalarm信号更新时间 ngx_timer_resolution = ccf->timer_resolution;默认值是0
    { ngx_string("timer_resolution"), // 单位是秒
    ...
    // 用于实现共享锁，一般linux是支持原子锁的，所以该文件没有意义
    ...
    { ngx_string("debug_points"),
      ...
      &ngx_debug_points },
    // 设置worker进程运行的用户和用户组  user username [groupname],不设置groupname则group默认为username
    { ngx_string("user"),
    ...
    { ngx_string("worker_cpu_affinity"),
      ...
      0,
      NULL },
    // 设置worker进程打开文件描述符的最大个数 ，RLIMIT_NOFILE,进程可打开的最大文件描述符数量，超出将产生EMFILE错误
    ...
    { ngx_string("env"),
      ...
      0,
      NULL },
    // 加载动态模块的指令，不能在http/event里使用，而且要在http/event之前，否则会因为modules_used不允许加载动态模块
...
// ngx_core_module的函数指针表，创建配置结构体，两个函数都在本文件内,只是简单地创建并初始化成员
...
// 1.10引入动态模块后此变量不再使用 ，模块计数器，声明在ngx_conf_file.h, 在main里统计得到总数
// ngx_uint_t          ngx_max_module;  // 模块总数

// 解析命令行的标志变量,ngx_get_options()设置，仅在本文件里使用，woker等进程无关
...
// 启动时的参数,ngx_get_options()设置，仅在本文件里使用，woker等进程无关，前三个是u_char是因为要赋值给ngx_str_t，ngx_signal用char*是因为要做字符串比较，不需要存储
...
main(int argc, char *const *argv)
{
    ...
    // 解析命令行参数,在本文件中查找ngx_get_options,设置ngx_show_version/ngx_show_help等变量
    ...
    if (ngx_show_version) {
        ...
        if (!ngx_test_config) { //如果是-t参数，表示接下来要走流程检查配置但不启动
            return 0;
        }
    }
     // 在ngx_os_init函数里设置（os/unix/ngx_posix_init.c）,使用系统调用getrlimit(RLIMIT_NOFILE, &rlmt),使nginx能够打开的最多描述数量，但似乎并没有使用
    /* TODO */ ngx_max_sockets = -1;

    // 初始化nginx环境的当前时间，ngx_times.c,初始化各个cache时间变量，调用ngx_time_update()，得到当前的时间
    ngx_time_init();    
    ...
    // 获取当前进程也就是master进程的pid,如果是master/worker，会fork出新的子进程，见os/unix/ngx_daemon.c
    ngx_pid = ngx_getpid();
    // 1.13.8新增,获取父进程pid
    ngx_parent = ngx_getppid();
    // 初始化log，仅在配置阶段使用,ngx_prefix是-p后的参数，即nginx的工作目录,默认是NGX_CONF_PREFIX，即/usr/local/nginx
    ...
    // 创建cycle使用的内存池，用于之后所有的内存分配，必须成功,这个内存池很小，只有1k，因为只是临时用
    init_cycle.pool = ngx_create_pool(1024, log);
    if (init_cycle.pool == NULL) {
        return 1;
    }
    // 分配内存，拷贝参数，没有使用内存池,拷贝到全局变量ngx_argc/ngx.argv
    ...
    ngx_slab_sizes_init();
    // 检查NGINX环境变量，获取之前监听的socket,用于update binary
    if (ngx_add_inherited_sockets(&init_cycle) != NGX_OK) {
        return 1;
    }
    // 开始计算所有的静态模块数量,ngx_modules是nginx模块数组，存储所有的模块指针，由make生成在objs/ngx_modules.c
    // 这里赋值每个模块的index成员, ngx_modules_n保存了最后一个可用的序号,ngx_max_module是模块数量的上限
    if (ngx_preinit_modules() != NGX_OK) {
        return 1;
    }
    // ngx_cycle.c,初始化cycle,800多行,由之前最基本的init_cycle产生出真正使用的cycle,解析配置文件，配置所有的模块,创建共享内存，打开文件，监听配置的端口
    ...
    // 定义在ngx_cycle.c,如果用了-t参数要测试配置，在这里就结束了
    if (ngx_test_config) {
        if (!ngx_quiet_mode) {
    ...
    if (ngx_signal) {
        // 定义在ngx_cycle.c,最后调用os/unix/ngx_process.c里的函数ngx_os_signal_process()
        return ngx_signal_process(cycle, ngx_signal);
    }
    // ngx_posix_init.c,使用NGX_LOG_NOTICE记录操作系统的一些信息,通常不会显示
    ngx_os_status(cycle->log);
    // 定义在ngx_cycle.c,
    // volatile ngx_cycle_t  *ngx_cycle;nginx生命周期使用的超重要对象,指针切换到ngx_init_cycle()创建好的新对象
    ngx_cycle = cycle;
    // ngx_init_cycle()里已经解析了配置文件,检查core模块的配置
    ccf = (ngx_core_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_core_module);
    // master on且单进程,如果master_process off那么就不是master进程,ngx_process定义在os/unix/ngx_process_cycle.c
...
// unix/linux将进程守护进程化
#if !(NGX_WIN32)
    // os/unix/ngx_process.c 使用signals数组，初始化信号处理handler
    if (ngx_init_signals(cycle->log) != NGX_OK) {
        return 1;
    }
    // 守护进程
    if (!ngx_inherited && ccf->daemon) {
        // os/unix/ngx_daemon.c 经典的daemon操作，使用fork
...
#endif
    // ngx_cycle.c,把ngx_pid字符串化，写入pid文件, 在daemon后，此时的pid是真正的master进程pid
    ...
    // 默认不使用标准流记录日志,在ngx_log.c里
    ngx_use_stderr = 0;
    // 启动单进程或者master/worker多进程，内部会调用fork,子进程完全复制父进程的cycle，包括打开的文件、共享内存、监听的端口
    if (ngx_process == NGX_PROCESS_SINGLE) {
        // 如果master_process off那么就不是master进程,定义在ngx_process_cycle.c
        ngx_single_process_cycle(cycle);

    } else {
        // 启动worker进程，数量由配置决定，即worker_processes指令，核心操作是sigsuspend，暂时挂起进程，不占用CPU，只有收到信号时才被唤醒,定义在ngx_process_cycle.c
...
ngx_show_version_info(void)
{
    ...
    if (ngx_show_help) {
        ngx_write_stderr(
            ...
            "  -T            : test configuration, dump it and exit"            // -T参数,可以dump整个配置文件
                               NGX_LINEFEED
            "  -q            : suppress non-error messages "
                               "during configuration testing" NGX_LINEFEED      // 通过-t测试配置文件是否错误的时候,nginx -t -q可以把error级别以下的日志不输出到屏幕
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    ngx_int_t         s;
    ngx_listening_t  *ls;
    // 获取环境变量，NGINX_VAR定义在nginx.h，值是"NGINX",参数envvar为环境变量的名称，如果该变量存在则会返回指向该内容的指针
    ...
    // 从环境变量字符串里切分出socket,逐个添加进cycle->listening数组
    for (p = inherited, v = p; *p; p++) { // 遍历环境变量
        ...
        if (*p == ':' || *p == ';') {
            ...
            ls->inherited = 1;  // 表示已经的得到要继承的socket
        }
    }
    // v和p是环境变量字符串的指针，最后必须都到末尾,否则格式有错误，但只记录日志，正常添加socket描述符
    if (v != p) {
        ngx_log_error(NGX_LOG_EMERG, cycle->log, 0,
                      "invalid socket number \"%s\" in " NGINX_VAR
    ...
    // 设置继承socket描述符的标志位
    ngx_inherited = 1;
    // 定义在 ngx_connection.c ,根据传递过来的socket描述符，使用系统调用获取之前设置的参数,填入ngx_listeing_t结构体
...
tz_found:
    ...
    for (i = 0; i < ccf->env.nelts; i++) {
        ...
        if (var[i].data[var[i].len] == '=') {// 当kye=   =号后面没有值则continue
            n++;
            continue;
        }
 ...
 * */
// 执行新的nginx程序，热更新，使用环境变量传递已经打开的文件描述符
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            case 'p':
                if (*p) {
                    ngx_prefix = p;  // -p后紧接着路径,设置ngx_prefix变量
                    goto next;
                }
...
// 调用ngx_process_options()初始化ngx_cycle的prefix,设置cycle->prefix/cycle->conf_prefix/conf_param等成员
static ngx_int_t
...
ngx_core_module_create_conf(ngx_cycle_t *cycle)
{
     ...
     */
    // 需要设置初始值的参数必须置为unset,ngx_conf_init_value系列宏只识别unset
...
ngx_core_module_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    ngx_conf_init_msec_value(ccf->shutdown_timeout, 0);// worker进程关闭的超时时间，默认永远等待
...
ngx_set_cpu_affinity(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    value = cf->args->elts;
    // 处理auto参数, auto后可以再使用一个掩码，限定绑定的cpu
...
ngx_load_module(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    // 使用"ngx_module_names"获取动态库里的模块名字数组
    ...
    // 使用"ngx_module_order"获取取动态库里的模块顺序数组
    // 模块顺序只对http filter模块有意义 所以可以没有，不需要检查
    order = ngx_dlsym(handle, "ngx_module_order");
    // 遍历动态库里的模块数组,流程类似ngx_preinit_modules,调用ngx_add_module添加模块
    for (i = 0; modules[i]; i++) {
        module = modules[i];
        // 在这里为动态模块设置名字
        module->name = names[i];
        // cycle->modules_n是模块计数器 如果超过最大数量则报错,使用模块里的各种信息进行检查，只有正确的才能加载
...
#if (NGX_HAVE_DLOPEN)
// 调用dlclose关闭动态库 #define ngx_dlclose(handle)        dlclose(handle)
static void

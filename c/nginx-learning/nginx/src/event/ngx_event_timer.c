...
// 定时器红黑树,键值是超时时间(毫秒时间戳),里面使用ngx_event_t.timer成员组织为红黑树,timedout表示已经超时,timer_set表示已经加入定时器红黑树
ngx_rbtree_t              ngx_event_timer_rbtree;

// 定时器红黑树的哨兵节点,哨兵节点是所有最下层的叶子节点都指向一个NULL空节点
...
// 初始化定时器
// 实际上就是初始化一个红黑树结构,注意树的插入函数是 ngx_rbtree_insert_timer_value
ngx_int_t
...
ngx_event_expire_timers(void)
{
    ...
    for ( ;; ) {
        ...
        ev->handler(ev);// 超时的时候调用读写事件回调函数,从而在里面判断timedout标志位
    }
}

...
// 表示对应的连接上有数据可以读出(TCP连接的远端主动关闭连接,也相当于可读事件,因为需要处理发送来的FIN包) 
#define EPOLLIN        0x001 

// 表示对应的连接上有紧急数据需要读 
#define EPOLLPRI       0x002

// 表示对应的连接上可以写入数据发送(主动向上游服务器发起非阻塞的TCP连接,连接建立成功的事件相当于可写事件)
...
// EPOLLRDHUP表示客户端关闭连接(断连),也当做读事件处理,这时recv返回0
#define EPOLLRDHUP     0x2000

// 设置该标记后读取完数据后,如果正在处理数据过程中又有新的数据到来,不会触发epoll_wait返回,除非数据处理完毕后重新add epoll_ctl操作
#define EPOLLEXCLUSIVE 0x10000000

// 表示对这个事件只处理一次,下次需要处理时需重新加入epoll 
#define EPOLLONESHOT   0x40000000

// 表示将触发方式设置妁边缘触发(ET),系统默认为水平触发(LT) 
...
// epoll系统调用使用的结构体,nginx只使用ptr,存储连接对象的指针,指针最低位用做标志位,存储instance
...
// 等待注册的事件发生,内核把发生的事件拷贝到events数组里,返回值是数量
...
// epoll模块的配置结构体
typedef struct {
    // epoll系统调用,获取事件的数组大小,对应指令epoll_events
...
// epoll添加事件
// 检查事件关联的连接对象,决定是新添加还是修改,避免误删除了读写事件的关注
static ngx_int_t ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event,
    ngx_uint_t flags);

// epoll删除事件
// 检查事件关联的连接对象,决定是完全删除还是修改,避免误删除了读写事件的关注
...
// 使用epoll模拟了事件通知机制
// 向文件里写一个数字,令文件可读,从而触发epoll事件,参数handler是真正的业务回调函数
#if (NGX_HAVE_EVENTFD)
static ngx_int_t ngx_epoll_notify(ngx_event_handler_pt handler);
#endif

// epoll模块核心功能,调用epoll_wait处理发生的事件
// 使用event_list和nevents获取内核返回的事件,timer是无事件发生时最多等待的时间,即超时时间
// 函数可以分为两部分:
//                一是用epoll获得事件
//                二是处理事件,加入延后队列
...
// epoll系统调用使用的数组,存储内核返回的事件,大小由nevents确定
...
static ngx_event_module_t  ngx_epoll_module_ctx = {
    ...
    // epoll的事件模块访问接口,是一个函数表    
    {
        ...
        // 调用epoll_create初始化epoll机制, timer无意义
        // 参数size=cycle->connection_n / 2,但并无实际意义
        // 设置全局变量，操作系统提供的底层数据收发接口,初始化全局的事件模块访问接口，指向epoll的函数
        // 默认使用et模式,边缘触发,高速    
...
// 获取已经完成的异步I/O事件,相当于epoll中的epoll_wait方法
static int
...
// 初始化aio事件列表,ngx_file_aio_read添加读文件事件,当读取完毕后epoll会触发ngx_epoll_eventfd_handler->ngx_file_aio_event_handler 
// nginx file aio只提供了read接口,不提供write接口,因为异步aio只从磁盘读和写,而非aio方式一般写会落到磁盘缓存,所以不提供该接口,如果异步io写可能会更慢
static void
ngx_epoll_aio_init(ngx_cycle_t *cycle, ngx_epoll_conf_t *epcf)
{
    ...
    // 设置用于异步I/O完成通知的ngx_eventfd_event事件,它与ngx_connection_t连接是对应的
    ngx_eventfd_event.data = &ngx_eventfd_conn;
    
    // 该函数在ngx_epoll_process_events中执行,在异步I/O事件完成后,使用ngx_epoll_eventfd handler方法处理
    ...
    //  gx_epoll_aio init方法会把异步I/O与epoll结合起来,当某一个异步I/O事件完成后,ngx_eventfd旬柄就处于可用状态,这样epoll_wait在返回ngx_eventfd event事件后就会调用它的回调方法ngx_epoll_eventfd handler处理已经完成的异步I/O事件
...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    if (ep == -1) {
        // 创建epoll句柄 ,参数size = cycle->connection_n / 2, 但并无实际意义
    ...
    // 设置全局变量,操作系统提供的底层数据收发接口,ngx_posix_init.c里初始化为linux的底层接口
    ngx_io = ngx_os_io;
    ...
    while (ready) {
        ...
        if (events > 0) {
            ...
            for (i = 0; i < events; i++) {
                ...
                // data成员指向这个异步I/O事件对应着的实际事件,这里的ngx_event_t为ngx_event_aio_t->event和ngx_epoll_eventfd_handler对应,先执行ngx_file_aio_read想异步I/O中添加读事件,然后通过epoll触发返回读取数据成功,再执行ngx_epoll_eventfd_handler
                // 和ngx_file_aio_read中io_submit对应
